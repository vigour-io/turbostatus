...
exports.info = (folder, remotes) => {
  var info = { folderName: folder.match(/([^\/]*)\/*$/)[1] }
  return Promise.resolve().then(() => {
    if (remotes) {
      spinner = ora(colors.blue.bold(' ' + info.folderName)).start()
      return updateRemotes(folder).then(() => {
        spinner.stop()
      })
    } else {
      return true
    ...
    describe(folder).then((result) => { info.describe = result })
  ]).then(() => info))
}
